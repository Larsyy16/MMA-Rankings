...
export default function Fighters({weightClass}) {
  ...
  const filteredRankings = mmaRankings.filter(ranking => 
    ranking.name === weightClass)


  const mmaClasses = filteredRankings.length > 0 ? filteredRankings[0].competitor_rankings.map((ranking,index) => (
